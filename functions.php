...
function limpiarDatos($datos){
	...
	// Quita las barras / escapandolas con comillas
...
function limpiarSession(){
	if (isset($_SESSION["nopost"])){
		$_SESSION["nopost"] = null;
		unset($_SESSION["nopost"]);
	}
}
# Funcion para obtener los post determinando cuantos queremos traer por pagina.
# Return: Los post dependiendo de la pagina que esta y cuantos post por pagina estableci.
...
function numero_paginas($post_por_pagina, $conexion){
	//4.- Calcula el numero de filas / articulos que devuelve la consulta
	$total_post = $conexion->prepare('SELECT FOUND_ROWS() as total');
	$total_post->execute();
	$total_post = $total_post->fetch()['total'];

	//5. calcula el numero de paginas que habra en la paginacion
